...
Later, an Outlook-opened reset link reached Supabase's `/verify` endpoint, but the new browser did not have the PKCE verifier stored in the browser that requested the email. Function version 7 sends Payroll recovery and invitation links directly to the deployed password page with `token_hash` and `type` parameters. That page already calls `verifyOtp` and removes the token from browser history after verification. Links for other applications still use Supabase `/verify`. Tests cover the Payroll recovery/invite link and the other-application fallback. Request one new email after version 7; links generated before this deployment keep the old format and will not be repaired. Do not share the full reset URL or token in screenshots or support logs.
